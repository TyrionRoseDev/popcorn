...
function makeFeedItem(
	overrides: Partial<FeedItem> & { tmdbId: number },
): FeedItem {
	return {
		mediaType: "movie",
...
describe("interleaveShuffleFeed", () => {
	it("interleaves three sources at 50/30/20 ratio", () => {
		const taste = Array.from({ length: 10 }, (_, i) =>
			makeFeedItem({ tmdbId: i + 1 }),
		);
		const trending = Array.from({ length: 6 }, (_, i) =>
			makeFeedItem({ tmdbId: i + 100, isTrending: true }),
		);
		const discovery = Array.from({ length: 4 }, (_, i) =>
			makeFeedItem({ tmdbId: i + 200 }),
		);
		...
		const trendingCount = result.filter(
			(item) => item.tmdbId >= 100 && item.tmdbId < 200,
		).length;
	...
	it("handles empty sources gracefully", () => {
		const taste = Array.from({ length: 5 }, (_, i) =>
			makeFeedItem({ tmdbId: i + 1 }),
		);
