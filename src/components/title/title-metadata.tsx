      ...
      <div className="flex flex-col gap-2.5">
        {director && (
          <div className="flex gap-2 text-sm">
            <span className="text-cream/40 min-w-[80px] font-mono-retro text-xs">Director</span>
            <span className="text-cream/80">{director}</span>
          </div>
        )}
        <div className="flex gap-2 text-sm">
          <span className="text-cream/40 min-w-[80px] font-mono-retro text-xs">Rating</span>
          <span className="text-cream/80">
            <span className="text-neon-amber [text-shadow:0_0_6px_rgba(255,184,0,0.4)]">★</span>{" "}
            {rating.toFixed(1)} / 10
          </span>
        </div>
        {seasons != null && (
          <div className="flex gap-2 text-sm">
            <span className="text-cream/40 min-w-[80px] font-mono-retro text-xs">Seasons</span>
            <span className="text-cream/80">{seasons}</span>
          </div>
        )}
        {episodes != null && (
          <div className="flex gap-2 text-sm">
            <span className="text-cream/40 min-w-[80px] font-mono-retro text-xs">Episodes</span>
            <span className="text-cream/80">{episodes}</span>
          </div>
        )}
        {status && (
          <div className="flex gap-2 text-sm">
            <span className="text-cream/40 min-w-[80px] font-mono-retro text-xs">Status</span>
