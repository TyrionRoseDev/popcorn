...
export function RewatchConfirmModal({
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent
				className="max-w-sm border-none bg-transparent p-0 gap-0 shadow-none"
				overlayClassName="bg-black/80 backdrop-blur-sm"
				showCloseButton={false}
				onOpenAutoFocus={(e) => e.preventDefault()}
			>
				<DialogTitle className="sr-only">Start Rewatch</DialogTitle>
				<div
				...
				>
					...
					<h2 className="font-display text-xl tracking-wide text-cream leading-tight">
						Start rewatching <span className="text-neon-pink">{titleName}</span>
						?
					</h2>
					...
					{isComplete ? (
						<p className="text-sm text-cream/40 leading-relaxed max-w-[280px]">
							Your progress and notes from {watchLabel} are saved. You'll start
							fresh tracking episodes again.
						</p>
					) : (
						<p className="text-sm text-cream/40 leading-relaxed max-w-[280px]">
